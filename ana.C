...
void ana(int sample=0){
  ...
  if(sample==0){
    ...
    chain->Add("inputs/TTZToLLNuNu.root");
    //Set Names of outputfiles
    hstfilename = "Outputs/hst_TTZ.root";
    sumfilename = "Outputs/sum_TTZ.txt";
    //Set some options
    m_selec.SetData(0); //MC=0, data=1
    m_selec.SetYear(2016);
  }
  if(sample==1){
    chain->Add("inputs/WZTo3LNu.root");
    hstfilename = "Outputs/hst_WZ.root";
    sumfilename = "Outputs/sum_WZ.txt";
    m_selec.SetData(0); //MC=0, data=1
    m_selec.SetYear(2016);
  }
  if(sample==2){
    chain->Add("inputs/ZZTo4L.root");
    hstfilename = "Outputs/hst_ZZ.root";
    sumfilename = "Outputs/sum_ZZ.txt";
    m_selec.SetData(0); //MC=0, data=1
    m_selec.SetYear(2016);
  } 
